...
```bash
npm install
echo "GROQ_API_KEY=gsk_..." > .env.local   # required for the LLM call
npm run dev
```
...
- Tailwind v4
- Groq (`llama-3.3-70b-versatile`) for the rebuttal generation — fast and free at this size; can swap to Claude or another provider via `src/lib/llm.ts`
